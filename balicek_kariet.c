...
void inicializuj_balicek(Karta volne[], int *pocet_volnych_kariet) {
    ...
    *pocet_volnych_kariet = POCET_KARIET_V_BALICKU;
}
...
void rozdaj_karty_hracom(Hra *hra) {
    ...
    int pocet_kariet = hra->pocet_volnych_kariet - 1;
...
void presun_karty_z_kopy_do_volnych(Hra *hra) {
    ...
    // Karta na vrchu ostáva v kope
    hra->kopa[0] = hra->kopa[hra->pocet_kariet_v_kope - 1];
    hra->pocet_kariet_v_kope = 1; 

    printf("Karty z kopy boli presunuté do voľných kariet.\n");
}